...
def get_predictions(
    model, tokenizer, data_loader, num_beams=4, max_length=32, length_penalty=1
):
    model.to(device)
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader):
            ...
            prediction = [
                tokenizer.decode(ids, skip_special_tokens=True) for ids in outs
            ]
...
def main():
    ...
    loader = torch.utils.data.DataLoader(
        valid_dataset, batch_size=32, collate_fn=collator
    )
